...
export function Podio({ inicial, tamano }: { inicial: PersonaPodio[]; tamano: number }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-[clamp(10px,1.5vh,18px)] py-[clamp(14px,2vh,24px)]">
      <div className="grid min-h-0 flex-1 grid-cols-[.94fr_1.12fr_.94fr] items-stretch gap-[clamp(12px,1.7vw,28px)]">
        {principales.map((persona) => {
          ...
          const estilo = primero
            ? "border-4 border-amber-300 bg-gradient-to-br from-amber-300/35 via-white/25 to-[var(--epm-verde)]/20 shadow-[0_0_60px_rgba(251,191,36,.38)] ring-4 ring-amber-200/20"
            : puesto === 2
              ? "border-2 border-slate-200/70 bg-white/13"
              : "border-2 border-orange-300/55 bg-orange-200/10";
          return (
            <article key={persona.id} className={`relative flex h-full min-h-0 flex-col overflow-hidden rounded-[clamp(20px,2vw,34px)] p-[clamp(14px,1.7vw,28px)] shadow-2xl backdrop-blur-md ${estilo}`}>
              ...
              <div className="relative z-10 flex shrink-0 items-center justify-between gap-3">
                ...
                <span className={`grid h-[clamp(42px,4vw,64px)] w-[clamp(42px,4vw,64px)] shrink-0 place-items-center rounded-full font-display text-[clamp(21px,2.1vw,34px)] font-extrabold shadow-xl ${primero ? "bg-gradient-to-br from-yellow-200 to-amber-500 text-amber-950" : puesto === 2 ? "bg-slate-200 text-slate-700" : "bg-orange-400 text-orange-950"}`}>{puesto}</span>
              </div>
              <div className="relative z-10 grid min-h-0 flex-1 place-items-center py-[clamp(8px,1.4vh,20px)]">
                <div className={primero ? "rounded-full bg-gradient-to-br from-yellow-200 via-amber-400 to-yellow-600 p-[clamp(5px,.55vw,9px)] shadow-[0_0_35px_rgba(251,191,36,.55)]" : puesto === 2 ? "rounded-full bg-gradient-to-br from-white to-slate-400 p-1.5" : "rounded-full bg-gradient-to-br from-orange-200 to-orange-600 p-1.5"}>
                  <FotoCircular src={persona.urlFoto} alt={`Foto de ${persona.nombre}`} className={primero ? "h-[clamp(165px,24vh,290px)] w-[clamp(165px,24vh,290px)] border-4" : "h-[clamp(145px,21vh,255px)] w-[clamp(145px,21vh,255px)] border-4"} />
                </div>
              </div>
              <div className="relative z-10 w-full rounded-2xl bg-black/10 p-[clamp(12px,1.25vw,20px)]">
                <h2 className={`${primero ? "text-[clamp(25px,2.5vw,42px)]" : "text-[clamp(20px,2vw,34px)]"} font-extrabold leading-tight`}>{persona.nombre}</h2>
                ...
                <strong className={`mt-3 block text-right font-display leading-none ${primero ? "text-[clamp(40px,4.5vw,72px)] text-amber-300 drop-shadow" : "text-[clamp(31px,3.5vw,56px)] text-[var(--epm-verde)]"}`}>{persona.puntosTotales.toLocaleString("es-CO")} <small className="text-[.34em] text-white/70">pts</small></strong>
              </div>
            </article>
          );
        })}
