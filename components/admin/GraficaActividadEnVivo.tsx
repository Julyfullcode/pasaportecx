...
export function GraficaActividadEnVivo({ id }: { id: string }) {
  ...
  return <section className="tarjeta p-5 md:p-7">
    ...
    {datos.contexto && <div className="mt-4 rounded-2xl border border-sky-100 bg-sky-50/70 p-4"><p className="text-xs font-extrabold uppercase tracking-wider text-[var(--epm-azul)]">Descripción o contexto</p><p className="mt-2 whitespace-pre-line leading-relaxed text-slate-700">{datos.contexto}</p></div>}
    {datos.respuestaCorrecta && <div className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-extrabold uppercase tracking-wider text-emerald-700">Respuesta correcta</p><p className="mt-2 font-bold leading-relaxed text-emerald-950">{datos.respuestaCorrecta}</p></div>}
