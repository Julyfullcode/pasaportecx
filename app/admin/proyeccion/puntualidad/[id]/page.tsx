...
import { esConfiguracionPuntualidad, fechaHoraPuntualidadLegible } from "@/lib/puntualidad";
...
export default async function ProyeccionPuntualidad({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!desafio || !esConfiguracionPuntualidad(desafio.configuracion)) notFound();
  ...
  return (
    <MarcoProyeccion primera="Llegada a tiempo" segunda={desafio.titulo}>
      <QrPuntualidadDinamico
        ...
        hora={fechaHoraPuntualidadLegible(desafio.configuracion)}
      />
    </MarcoProyeccion>
  );
}
