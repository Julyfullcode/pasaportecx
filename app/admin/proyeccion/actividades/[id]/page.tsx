...
export default async function ProyectarRespuestasActividad({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="marca-gradiente flex h-screen flex-col overflow-hidden p-5 text-white md:p-8"><ActualizacionModeracion />
    <header className="flex shrink-0 items-center gap-5 border-b border-white/20 pb-5"><Logo className="h-12 w-auto md:h-16" /><span className="h-12 w-px bg-white/25" /><div className="min-w-0"><p className="font-extrabold uppercase tracking-widest text-[var(--epm-verde)]">Matriz de resultados anónimos</p><h1 className="truncate text-3xl font-extrabold md:text-5xl">{actividad.titulo}</h1></div><span className="ml-auto hidden shrink-0 rounded-full bg-white/15 px-5 py-3 font-extrabold md:inline-flex">{evaluaciones.length} evaluaciones</span></header>
