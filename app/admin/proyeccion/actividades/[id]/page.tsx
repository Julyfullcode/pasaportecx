...
export default async function ProyectarRespuestasActividad({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="marca-gradiente flex h-screen flex-col overflow-hidden p-5 text-white md:p-8"><ActualizacionModeracion />
    ...
    {evaluaciones.length ? <div className="flex min-h-0 flex-1 flex-col">
      <section className="matriz-scroll mt-4 min-h-0 flex-1 overflow-scroll rounded-3xl bg-white shadow-2xl">
        <table className="min-h-full table-fixed border-separate border-spacing-0 text-slate-800" style={{ width: `${320 + Math.max(evaluaciones.length, 3) * 300}px` }}>
          <colgroup><col className="w-[320px]" />{evaluaciones.map((respuestas) => <col key={respuestas[0].participanteId} className="w-[300px]" />)}</colgroup>
          <thead><tr><th className="sticky left-0 top-0 z-30 w-[320px] border-b border-r bg-white p-[clamp(8px,1vw,16px)] text-left text-[clamp(18px,1.45vw,27px)] font-extrabold text-[var(--epm-azul-profundo)]">Preguntas</th>{evaluaciones.map((respuestas, indice) => { const empresa = empresaPorId.get(respuestas.find((respuesta) => respuesta.empresaEvaluadaId)?.empresaEvaluadaId ?? ""); return <th key={respuestas[0].participanteId} className="sticky top-0 z-20 border-b border-r bg-slate-50 p-[clamp(6px,.8vw,13px)] text-center last:border-r-0">{empresa?.urlLogo ? <img src={empresa.urlLogo} alt={`Logo de ${empresa.nombre}`} className="mx-auto h-[clamp(24px,3.5vh,48px)] max-w-full object-contain" /> : <span className="mx-auto grid h-9 w-9 place-items-center rounded-full bg-sky-100 text-sm font-extrabold text-[var(--epm-azul)]">{indice + 1}</span>}<strong className="mt-1 block truncate text-[clamp(9px,.75vw,13px)] text-[var(--epm-azul-profundo)]">{empresa?.nombre ?? "Sin empresa"}</strong></th>; })}</tr></thead>
          <tbody>{preguntas.map((pregunta, indicePregunta) => <tr key={pregunta.id}><th className={`sticky left-0 z-10 w-[320px] border-b border-r p-[clamp(7px,.8vw,13px)] text-left align-middle text-[clamp(10px,.78vw,14px)] font-extrabold leading-snug text-[var(--epm-azul-profundo)] ${indicePregunta % 2 ? "bg-sky-50" : "bg-white"}`}>{pregunta.titulo}</th>{evaluaciones.map((respuestas) => { const respuesta = respuestas.find((item) => item.preguntaId === pregunta.id); return <td key={respuestas[0].participanteId} className={`overflow-hidden whitespace-pre-line border-b border-r p-[clamp(7px,.8vw,13px)] align-middle text-[clamp(8px,.68vw,12px)] leading-snug last:border-r-0 ${indicePregunta % 2 ? "bg-sky-50/50" : "bg-white"}`}>{respuesta ? textoRespuesta(respuesta.respuesta) : <span className="text-slate-400">Sin respuesta</span>}</td>; })}</tr>)}</tbody>
        </table>
      </section>
