...
export default async function AdminRecuerdos({
  ...
  const recuerdos = await db.recuerdo.findMany({
    ...
    include: { participante: true, reacciones: { select: { tipo: true } } },
  });
  return (
    <div className="p-4 md:p-7">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <a href="/api/recuerdos/zip" className="boton-secundario"><Download /> Descargar recuerdos en PNG</a>
      </div>
          ...
          <article key={recuerdo.id} className="tarjeta overflow-hidden">
            ...
            <div className="p-3"><p className="truncate text-sm font-extrabold">{recuerdo.participante.nombre}</p><p className="truncate text-xs text-slate-500">{recuerdo.descripcion}</p>
              <div className="mt-3 flex items-center justify-between">
                <a href={`/api/recuerdos/${recuerdo.id}/png`} className="grid h-10 w-10 place-items-center rounded-full bg-slate-100" aria-label="Descargar recuerdo en PNG"><Download size={17} /></a>
