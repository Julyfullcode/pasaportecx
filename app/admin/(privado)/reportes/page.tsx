...
export default function Reportes() {
  return (
    <div className="p-4 md:p-7">
      <div><p className="font-extrabold text-[var(--epm-verde-medio)]">Datos para el cierre</p><h1 className="text-3xl font-extrabold text-[var(--epm-azul-profundo)]">Reportes</h1><p className="mt-2 text-sm text-slate-600">Archivos de Excel listos para descargar, filtrar y analizar.</p></div>
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {reportes.map(([tipo, titulo, texto]) => (
          <a key={tipo} href={`/api/reportes/${tipo}`} className="tarjeta group p-5">
            ...
            <span className="mt-4 flex items-center gap-2 text-sm font-extrabold text-[var(--epm-azul)]"><Download size={17} /> Descargar Excel</span>
          </a>
        ))}
