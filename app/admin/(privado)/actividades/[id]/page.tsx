...
export default async function ConfigurarActividad({ params }: { params: Promise<{ id: string }> }) {
  ...
  const actividad = await db.actividad.findUnique({ where: { id }, include: { _count: { select: { respuestas: true } } } });
  ...
  if (actividad.tipo === TIPO_UNIVERSO_ARQUETIPOS) {
    ...
    return <div className="p-4 md:p-7"><Link href="/admin/actividades" className="inline-flex items-center gap-2 font-extrabold text-[var(--epm-azul)]"><ArrowLeft size={18} /> Volver</Link><div className="mt-4 flex flex-wrap items-end justify-between gap-4"><div><p className="font-extrabold text-[var(--epm-verde-medio)]">Actividad independiente</p><h1 className="text-3xl font-extrabold text-[var(--epm-azul-profundo)]">Configurar El Universo de la Experiencia</h1><p className="mt-2 text-slate-600">Administra los retos y controla cuándo pueden participar las personas.</p></div><div className="flex flex-wrap gap-2">{actividad.estado !== "PUBLICADA" ? <form action={publicarActividad}><input type="hidden" name="id" value={id} /><button className="boton-primario"><Play size={18} /> Publicar actividad</button></form> : <form action={cerrarActividad}><input type="hidden" name="id" value={id} /><button className="boton-secundario"><Square size={18} /> Cerrar actividad</button></form>}<a href="/universo/galaxia?demo=1" target="_blank" rel="noopener noreferrer" className="boton-secundario"><MonitorPlay size={18} /> Probar galaxia</a></div></div><section className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-red-200 bg-gradient-to-r from-red-50 to-amber-50 p-5"><div><h2 className="text-lg font-extrabold text-[var(--epm-azul-profundo)]">Datos de respuestas</h2><p className="mt-1 text-sm text-slate-600"><strong>{actividad._count.respuestas}</strong> {actividad._count.respuestas === 1 ? "respuesta almacenada" : "respuestas almacenadas"}. Al borrarlas también se reinician los avances y puntos de esta actividad; los retos y su configuración se conservan.</p></div><ReiniciarActividad id={id} etiqueta="Borrar respuestas existentes" totalRespuestas={actividad._count.respuestas} destacado /></section><div className="mt-6"><EditorUniversoArquetipos actividadId={id} configuracion={configuracionUniverso} /></div></div>;
  }
