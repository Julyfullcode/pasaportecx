...
export default async function ModerarActividad({ params }: { params: Promise<{ id: string }> }) {
  ...
  const respuestaCorrectaActual = actual ? descripcionRespuestaCorrecta(actual) : "";
  ...
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-[var(--epm-gris-fondo)] p-4 md:p-7"><ActualizacionModeracion />
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[1fr_280px]">
      <section className="marca-gradiente overflow-hidden rounded-3xl p-6 text-white shadow-xl md:p-9"><p className="text-sm font-extrabold uppercase tracking-widest text-[var(--epm-verde)]">En pantalla ahora</p><h2 className="mt-3 text-3xl font-extrabold">{etapa}</h2><p className="mt-3 max-w-3xl text-lg text-white/90">{descripcionEtapa}</p>{actual && <div className="mt-4 max-w-4xl rounded-2xl bg-white/10 p-4"><p className="text-xs font-extrabold uppercase tracking-wider text-[var(--epm-verde)]">Descripción o contexto</p><p className="mt-2 whitespace-pre-line leading-relaxed text-white/85">{actual.contexto}</p></div>}{respuestaCorrectaActual && <div className="mt-3 max-w-4xl rounded-2xl border border-[var(--epm-verde)]/40 bg-black/10 p-4"><p className="text-xs font-extrabold uppercase tracking-wider text-[var(--epm-verde)]">Respuesta correcta</p><p className="mt-2 font-extrabold leading-relaxed text-white">{respuestaCorrectaActual}</p></div>}<div className="mt-6 flex flex-wrap gap-3">
