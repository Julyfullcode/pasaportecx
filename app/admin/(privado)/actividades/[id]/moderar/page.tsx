...
export default async function ModerarActividad({ params }: { params: Promise<{ id: string }> }) {
  ...
  const esAutonoma = esFormularioCompleto || esUniverso;
  ...
  const etapa = esUniverso ? "Universo de tarjetas abierto" : esFormularioCompleto ? "Formulario completo" : actividad.pasoActual === 0 ? "Invitación inicial" : actual ? `Pregunta ${actividad.pasoActual} de ${preguntas.length}` : "Pantalla de cierre";
  const descripcionEtapa = esUniverso ? "Cada participante puede activar la órbita, descubrir una de las tarjetas al azar y registrar su misión estelar." : esFormularioCompleto ? "Al publicar, las personas verán la empresa y todas las preguntas abiertas en una sola pantalla." : actual?.titulo ?? (actividad.pasoActual === 0 ? actividad.invitacion : actividad.cierre);
  ...
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-[var(--epm-gris-fondo)] p-4 md:p-7"><ActualizacionModeracion />
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[1fr_280px]">
      ...
      <aside className="tarjeta flex flex-col items-center p-5 text-center"><h2 className="font-extrabold text-[var(--epm-azul-profundo)]">QR de acceso</h2><img src={`/api/admin/actividades/${id}/qr`} alt={`QR de ${actividad.titulo}`} className="mt-3 aspect-square w-full max-w-56 rounded-2xl border bg-white object-contain p-2" /><a href={`/api/admin/actividades/${id}/qr`} target="_blank" rel="noopener noreferrer" className="boton-secundario mt-3 w-full"><ExternalLink size={17} /> Abrir QR grande</a></aside>
    </div>

    {!esUniverso && <div className="mt-6"><GraficaActividadEnVivo id={id} /></div>}
    {esFormularioCompleto && <div className="mt-6"><ResumenAnalisisActividad analisis={analisis} /></div>}
    <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_280px]">
      <section className="tarjeta p-5"><h2 className="text-xl font-extrabold text-[var(--epm-azul-profundo)]">{esUniverso ? "Misiones en órbita" : "Estado de respuestas"}</h2><div className="mt-4 space-y-2">{preguntas.map((pregunta, indice) => <div key={pregunta.id} className={`flex items-center gap-3 rounded-xl border p-3 ${!esAutonoma && actividad.pasoActual === indice + 1 ? "border-[var(--epm-azul)] bg-sky-50" : "border-slate-200"}`}><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white font-extrabold text-[var(--epm-azul)]">{indice + 1}</span><span className="min-w-0 flex-1"><strong className="block truncate">{esUniverso ? "Tarjetas descubiertas y reflexiones guardadas" : pregunta.titulo}</strong><small className="text-slate-500">{conteoPorPregunta.get(pregunta.id) ?? 0} respuestas</small></span>{(esAutonoma ? (conteoPorPregunta.get(pregunta.id) ?? 0) > 0 : actividad.pasoActual > indice + 1) && <CheckCircle2 className="text-emerald-600" />}</div>)}</div></section>
