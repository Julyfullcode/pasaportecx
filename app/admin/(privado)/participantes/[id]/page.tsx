...
export default async function DetalleParticipante({ params }: { params: Promise<{ id: string }> }) {
  ...
  const persona = await db.participante.findUnique({
    ...
    include: {
      ...
      actividadesCompletadas: { orderBy: { completadaEn: "desc" }, include: { actividad: true } },
  ...
  return (
    <div className="p-4 md:p-7">
      ...
      <header className="tarjeta mt-4 flex flex-wrap items-center gap-4 p-5">
        ...
        <div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h1 className="text-3xl font-extrabold">{persona.nombre}</h1>{persona.esStaff && <span className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-3 py-1 text-xs font-extrabold text-violet-700"><ShieldCheck size={15} /> Staff</span>}</div><p>{persona.empresa.nombre} · {persona.dependencia?.nombre ?? "Sin dependencia"} · {persona.equipo?.nombre ?? "Sin equipo"}</p><p className="font-bold text-[var(--epm-azul)]">{persona.correoAutorizado?.correo ?? "Correo no asociado"}</p><p className="mt-1 text-xs text-slate-500">Registro: {persona.creadoEn.toLocaleString("es-CO")}</p></div>
      ...
      <div id="detalle-puntos" className="mt-5 grid scroll-mt-6 gap-5 xl:grid-cols-2">
        ...
        <section className="tarjeta p-5"><h2 className="text-xl font-extrabold">Otros puntos</h2><div className="mt-3 space-y-2">{persona.puntosRegistro > 0 && <div className="flex justify-between border-b py-2 text-sm"><span><strong className="block">Registro en la herramienta</strong><small className="block text-slate-500">{persona.creadoEn.toLocaleString("es-CO")}</small></span><strong className="text-emerald-700">+{persona.puntosRegistro}</strong></div>}{persona.actividadesCompletadas.map((item) => <div key={item.id} className="flex justify-between border-b py-2 text-sm"><span><strong className="block">Actividad: {item.actividad.titulo}</strong><small className="block text-slate-500">{item.completadaEn.toLocaleString("es-CO")}</small></span><strong className="text-emerald-700">+{item.puntosOtorgados}</strong></div>)}{persona.ajustes.map((a) => <div key={a.id} className="flex justify-between border-b py-2 text-sm"><span>{a.motivo}<small className="block text-slate-500">{a.creadoEn.toLocaleString("es-CO")}</small></span><strong className={a.puntos < 0 ? "text-red-700" : "text-emerald-700"}>{a.puntos > 0 ? "+" : ""}{a.puntos}</strong></div>)}</div><h3 className="mt-6 font-extrabold">Recuerdos ({persona.recuerdos.length})</h3><div className="mt-2 grid grid-cols-4 gap-2">{persona.recuerdos.map((r) => <img key={r.id} src={r.urlMiniatura} alt={r.descripcion || "Recuerdo"} className="aspect-square rounded-xl object-cover" />)}</div></section>
      </div>
    </div>
  );
}
