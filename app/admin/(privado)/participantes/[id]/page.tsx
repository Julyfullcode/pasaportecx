  ...
  return (
    <div className="p-4 md:p-7">
      ...
      <div id="detalle-puntos" className="mt-5 grid scroll-mt-6 gap-5 xl:grid-cols-2">
        ...
        <section className="tarjeta p-5"><h2 className="text-xl font-extrabold">Otros puntos</h2><div className="mt-3 space-y-2">{persona.puntosRegistro > 0 && <div className="flex justify-between border-b py-2 text-sm"><span><strong className="block">Registro en la herramienta</strong><small className="block text-slate-500">{persona.creadoEn.toLocaleString("es-CO")}</small></span><strong className="text-emerald-700">+{persona.puntosRegistro}</strong></div>}{persona.ajustes.map((a) => <div key={a.id} className="flex justify-between border-b py-2 text-sm"><span>{a.motivo}<small className="block text-slate-500">{a.creadoEn.toLocaleString("es-CO")}</small></span><strong className={a.puntos < 0 ? "text-red-700" : "text-emerald-700"}>{a.puntos > 0 ? "+" : ""}{a.puntos}</strong></div>)}</div><h3 className="mt-6 font-extrabold">Recuerdos ({persona.recuerdos.length})</h3><div className="mt-2 grid grid-cols-4 gap-2">{persona.recuerdos.map((r) => <img key={r.id} src={r.urlMiniatura} alt={r.descripcion || "Recuerdo"} className="aspect-square rounded-xl object-cover" />)}</div></section>
      </div>
    </div>
  );
}
