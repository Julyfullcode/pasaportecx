  ...
  return (
    <div className="p-4 md:p-7">
      ...
      <header className="tarjeta mt-4 flex flex-wrap items-center gap-4 p-5">
        ...
        <div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h1 className="text-3xl font-extrabold">{persona.nombre}</h1>{persona.esStaff && <span className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-3 py-1 text-xs font-extrabold text-violet-700"><ShieldCheck size={15} /> Staff</span>}</div><p>{persona.empresa.nombre} · {persona.equipo?.nombre ?? "Sin equipo"}</p><p className="font-bold text-[var(--epm-azul)]">{persona.correoAutorizado?.correo ?? "Correo no asociado"}</p><p className="mt-1 text-xs text-slate-500">Registro: {persona.creadoEn.toLocaleString("es-CO")}</p></div>
